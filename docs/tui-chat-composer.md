...
- The `PasteBurst` state machine is bypassed entirely (no flicker suppression hold, no burst
  buffering, and no Enter suppression window).
- The key stream is treated as normal typing (including normal slash command behavior).
- Enabling the flag flushes any pending burst state (e.g. a held first ASCII char) so input is not
  dropped.
...
- **While actively buffering** (`paste_burst.is_active()`): `append_newline_if_active(now)` appends
  `\n` into the paste buffer.
- **Immediately after buffering** (enter suppression window): `newline_should_insert_instead_of_submit(now)`
  inserts `\n` into the textarea and calls `extend_window(now)` so a slightly-late Enter keeps
  behaving like “newline” rather than “submit”.
...
- Before applying the key, `PasteBurst::flush_before_modified_input` is used so buffered/pending
  text becomes an explicit paste.
...
The tests for this code are intentionally small and deterministic.
...
The `tui2` crate has corresponding tests in:
