...
use codex_windows_sandbox::logging::LOG_FILE_NAME;
...
fn lock_sandbox_dir(
    ...
    let entries = [
        ...
        (real_sid, FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE),
    ];
    let sandbox_entries: Vec<(Vec<u8>, u32)> = sandbox_user_sids
        .iter()
        .map(|sid| (sid.clone(), FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE | DELETE))
        .collect();
    unsafe {
