...
impl ShellCommandHandler {
    fn to_exec_params(
        ...
        ExecParams {
            command: shell.derive_shell_invocation(&script),
            cwd: turn_context.resolve_path(params.workdir.clone()),
            timeout_ms: params.timeout_ms,
            env: create_env(&turn_context.shell_environment_policy),
            with_escalated_permissions: params.with_escalated_permissions,
...
/// If `raw_command` is of the form `<shell> -lc "<script>"`, extract and return
/// the inner `<script>`. Otherwise, return `raw_command`.
...
mod tests {
    ...
    fn commands_generated_by_shell_command_handler_can_be_matched_by_is_known_safe_command() {
        ...
        assert!(is_known_safe_command(
            &bash_shell.derive_shell_invocation("ls -la"),
        ));
        ...
        assert!(is_known_safe_command(
            &zsh_shell.derive_shell_invocation("ls -la"),
        ));
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            assert!(is_known_safe_command(
                &powershell.derive_shell_invocation("ls -Name"),
            ));
        }
    }
    ...
    fn unwraps_nested_bash_lc_for_bash_shell() {
        ...
        assert_eq!(
            shell.derive_shell_invocation(&script),
            vec![
                "/bin/bash".to_string(),
                "-lc".to_string(),
                "echo hi".to_string()
