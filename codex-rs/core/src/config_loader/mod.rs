...
const DEFAULT_SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/codex/config.toml";
...
pub async fn load_config_layers_state(
    ...
    let system_config_toml_file = if cfg!(unix) {
        Some(AbsolutePathBuf::from_absolute_path(
            DEFAULT_SYSTEM_CONFIG_TOML_FILE_UNIX,
        )?)
    } else {
...
///   `create_entry` and returns `Ok(Some(entry))`.
/// - If the file does not exist, uses an empty Table as `toml::Value` with
///   `create_entry` and returns `Ok(Some(entry))`.
/// - If there is an error reading the file or parsing the TOML, returns
///   `Err(...)`.
async fn load_config_toml_for_required_layer(
    ...
    let toml_value = match tokio::fs::read_to_string(toml_file).await {
        Ok(contents) => {
            ...
            let user_config_parent = toml_file.parent().ok_or_else(|| {
                io::Error::new(
                    io::ErrorKind::InvalidData,
                    format!(
