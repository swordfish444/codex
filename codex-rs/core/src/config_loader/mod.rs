...
pub async fn load_config_layers_state(
    ...
    let session_override_keys = cli_overrides.iter().map(|(key, _)| key.clone()).collect();
    ...
    let system = match layers.managed_config {
        Some(cfg) => {
            ...
            Some(ConfigLayerEntry::new(
                ConfigLayerName::System { file: system_file },
                cfg,
            ))
        }
        None => None,
    ...
    Ok(ConfigLayerStack {
        user: ConfigLayerEntry::new(ConfigLayerName::User { file: user_file }, layers.base),
        session_flags: ConfigLayerEntry::new(
            ConfigLayerName::SessionFlags {
                override_keys: session_override_keys,
            },
            cli_overrides_layer,
        ),
        system,
