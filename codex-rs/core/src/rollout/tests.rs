...
async fn test_source_filter_excludes_non_matching_sessions() {
    ...
    let interactive_only = get_conversations(home, 10, None, INTERACTIVE_SESSION_SOURCES, None)
        .await
        .unwrap();
    ...
    let all_sessions = get_conversations(home, 10, None, NO_SOURCE_FILTER, None)
        .await
        .unwrap();
...
async fn test_model_provider_filter_selects_only_matching_sessions() -> Result<()> {
    ...
    let alpha_sessions = get_conversations(home, 10, None, NO_SOURCE_FILTER, Some("alpha")).await?;
    assert_eq!(alpha_sessions.items.len(), 1);
    let alpha_id_str = alpha_id.to_string();
    let alpha_head = alpha_sessions.items[0]
        .head
        .first()
        .and_then(|value| value.get("id"))
        .and_then(serde_json::Value::as_str);
    assert_eq!(alpha_head, Some(alpha_id_str.as_str()));

    let beta_sessions = get_conversations(home, 10, None, NO_SOURCE_FILTER, Some("beta")).await?;
    ...
    let unknown_sessions =
        get_conversations(home, 10, None, NO_SOURCE_FILTER, Some("unknown")).await?;
    assert!(unknown_sessions.items.is_empty());

    let all_sessions = get_conversations(home, 10, None, NO_SOURCE_FILTER, None).await?;
