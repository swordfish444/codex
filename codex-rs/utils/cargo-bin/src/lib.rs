...
pub enum CargoBinError {
    ...
    #[error(
        "could not locate binary {name:?}; tried env vars {env_keys:?} and fallback path {fallback:?}"
    )]
    NotFound {
        name: String,
        env_keys: Vec<String>,
...
pub fn cargo_bin(name: &str) -> Result<PathBuf, CargoBinError> {
    ...
    let fallback =
        fallback_target_dir_path(name).map_err(|source| CargoBinError::CurrentDir { source })?;
    if fallback.exists() {
        return Ok(fallback);
    }
...
fn fallback_target_dir_path(name: &str) -> Result<PathBuf, std::io::Error> {
    ...
    let profile = std::env::var_os("PROFILE").unwrap_or_else(|| "debug".into());
    Ok(target_dir
        .join(profile)
