...
"#,
    ...
    // Normally, this would be an exact match, but it might be followed by a
    // hint if `git config set advice.defaultBranchName false` has not been set.
    assert!(
        output.starts_with(&expected_prefix),
        "expected output `{output}` to start with `{expected_prefix}`"
    );
