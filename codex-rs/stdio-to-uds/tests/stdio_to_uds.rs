...
fn pipes_stdin_and_stdout_through_socket() -> anyhow::Result<()> {
    let workspace_dir = std::env::current_dir().context("failed to read current dir")?;
    let tmp_base = workspace_dir.join("target/stdio-to-uds-tests");
    fs::create_dir_all(&tmp_base).context("failed to create test temp base directory")?;
    let dir =
        tempfile::tempdir_in(&tmp_base).context("failed to create temp dir inside workspace")?;
