...
"#,
    ...
    let expected_file_path = AbsolutePathBuf::try_from(codex_home.path().join("config.toml"))?;
...
async fn config_batch_write_applies_multiple_edits() -> Result<()> {
    ...
    let expected_file_path =
        AbsolutePathBuf::try_from(codex_home.path().canonicalize()?.join("config.toml"))?;
