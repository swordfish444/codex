...
impl ChatComposer {
    ...
    /// # Explicit paste entry point
    ///
    /// This is the single place where paste text is integrated into the composer, both for:
    ///
    ...
    /// It is responsible for:
    ///
    /// - Inserting small pastes directly into the textarea,
    /// - Converting very large pastes into placeholders (expanded on submit), and
    /// - Clearing any burst-related Enter suppression so an explicit paste cannot affect the next
    ///   user Enter key.
    ...
    ///
    /// When enabling the flag we must not drop any pending transient state (notably the held first
    /// ASCII character used for flicker suppression), so we flush any buffered/pending content
    /// through the normal paste path and then fully clear the burst state.
    ...
    /// # Tick-driven burst flush
    ///
    /// This is called from a UI tick to turn paste-burst transient state into an explicit action:
    ///
    /// - If a burst times out, flush it via `handle_paste(String)`.
    /// - If only the first ASCII char was held (flicker suppression) and no burst followed, emit
    ///   it as normal typed input.
    ///
    /// Flush a pending burst if its timing window has elapsed.
    ///
    /// This is typically called from a UI tick so a single "held" ASCII char can be rendered even
    /// when it turns out not to be part of a paste burst.
    ...
    /// (actively buffering, has a non-empty buffer, or has a held first ASCII char).
    pub(crate) fn is_in_paste_burst(&self) -> bool {
        self.paste_burst.is_active()
    }

    /// # Recommended tick delay for tests
    ///
    /// This returns a delay that reliably crosses the `PasteBurst` timeout threshold so tests can
    /// avoid boundary flakiness.
    ...
    /// # Apply a due `PasteBurst` flush
    ///
    /// This is the internal "glue" that turns [`PasteBurst::flush_if_due`] results into concrete
    /// textarea mutations.
    ///
    /// Callers:
    ///
    ...
    ///
    /// This is the lowest-level keypath for keys that mutate the textarea. It is also where plain
    ...
    fn handle_input_basic(&mut self, input: KeyEvent) -> (InputResult, bool) {
        ...
        // For non-char input, flush any buffered burst before applying the input.
        // Otherwise `clear_window_after_non_char()` can leave buffered text without a timestamp
        // to time out against.
