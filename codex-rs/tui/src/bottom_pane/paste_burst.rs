...
//!   considered part of a single burst. This is also the hold window for `pending_first_char`.
...
//!
//! The `BeginBuffer` decision is primarily produced on paths that do *not* hold the first char
//! (non-ASCII/IME, and retro-grab scenarios). For ASCII, the common path is
//! `RetainFirstChar -> BeginBufferFromPending`, which avoids the need for retro-grabbing.
//!
