...
import { DARWIN_BASH_VARIANTS, LINUX_BASH_VARIANTS } from "./constants.js";
import { BashSelection, OsReleaseInfo } from "./types.js";

function supportedDetail(variants: { name: string }[]): string {
...
export function selectLinuxBash(bashRoot: string, info: OsReleaseInfo): BashSelection {
  ...
  const candidates: Array<{ variant: (typeof LINUX_BASH_VARIANTS)[number]; matchesVersion: boolean }> = [];
